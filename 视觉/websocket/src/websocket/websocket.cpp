...
#include <iomanip> // 添加此头文件以使用 std::fixed 和 std::setprecision
#include <fstream> // 添加此头文件以使用 std::ifstream
namespace websocket {
...
Websocket::Websocket(rclcpp::Node::SharedPtr &nh) : nh_(nh) {
  ...
  // 初始化 nucleus 话题的订阅者
  nucleus_sub_ = nh_->create_subscription<std_msgs::msg::Float32>(
      "nucleus", 10, std::bind(&Websocket::OnGetNucleus, this, std::placeholders::_1));
  ...
  if (image_type_ == "mjpeg") {
    ...
    RCLCPP_WARN(nh_->get_logger(), "Websocket HbMem not supported in ROS2 Humble, falling back to mjpeg");
    using_hbmem_image_ = false;
    image_sub_ = nh_->create_subscription<sensor_msgs::msg::CompressedImage>(
        image_topic_name_,
        10,
...
int Websocket::SendImageMessage(sensor_msgs::msg::CompressedImage::SharedPtr frame_msg) {
  ...
  // //解压缩图像
  // cv::Mat frame = cv::imdecode(cv::Mat(frame_msg->data), cv::IMREAD_COLOR);

  // // 添加文字到图像上
  // std::string textLabel = "value: "; // 标签文本
  // std::string textValue = "100"; // 这里替换为实际的value值
  // cv::Point textOrgLabel(0, 30); // 标签文本的起始坐标
  // cv::Point textOrgValue(160, 30); // 实际value文本的起始坐标，这里假设在标签下方

  // int fontFace = cv::FONT_HERSHEY_SIMPLEX;
  // double fontScale = 1;
  // cv::Scalar color(0, 0, 255); // 文本颜色，这里是蓝色(BGR: 0, 0, 255)

  // int thickness = 2; // 文本轮廓的粗细

  // // 在图像上绘制标签文本
  // cv::putText(frame, textLabel, textOrgLabel, fontFace, fontScale, color, thickness);

  // // 在图像上绘制value文本
  // cv::putText(frame, textValue, textOrgValue, fontFace, fontScale, color, thickness);

  // // 压缩图像
  // std::vector<uchar> buf;
  // cv::imencode(".jpg", frame, buf);
  // frame_msg->data.assign(buf.begin(), buf.end());
...
// 回调函数实现
void Websocket::OnGetNucleus(const std_msgs::msg::Float32::SharedPtr msg) {
  // 将消息中的值四舍五入到小数点后两位，然后赋值给 nucleus_value_
    nucleus_value_ = RoundToTwoDigits(msg->data);
    //RCLCPP_INFO(nh_->get_logger(), "Received and rounded nucleus value: %f", nucleus_value_);
    // 这里可以添加其他需要执行的代码
}
...
std::string textLabel = "Momen_value: ";
//std::string nucleus_value_str = std::to_string(nucleus_value_) + " uSv/h"; // 将nucleus_value转换为字符串并添加单位
std::stringstream ss;
ss << std::fixed << std::setprecision(2) << nucleus_value_;
std::string nucleus_value_str = ss.str() + " uSv/h";
std::string radiationLabel = "Radia_dete: ";
std::string existenceText = (nucleus_value_ >= 0.20) ? "exist" : "inexistence";

cv::Point textOrgLabel(0, 30); // 标签文本的起始坐标
cv::Point textOrgValue(150, 30); // 实际value文本的起始坐标
cv::Point textOrgRadiation(0, 60); // Nuclear_radiation_detection文本的起始坐标
cv::Point textOrgExistence(140, 60); // Existence文本的起始坐标
...
// 在图像上绘制Momentary_value标签文本
cv::putText(frame, textLabel + nucleus_value_str, textOrgLabel, fontFace, fontScale, color, thickness);

// 在图像上绘制Nuclear_radiation_detection标签文本
cv::putText(frame, radiationLabel, textOrgRadiation, fontFace, fontScale, color, thickness);

// 在图像上绘制Existence文本
cv::putText(frame, existenceText, textOrgExistence, fontFace, fontScale, color, thickness);
...
void Websocket::MessageProcess() {
  while (!map_stop_) {
    ...
    if (x3_smart_msg_.size() > 20) {
      RCLCPP_WARN(nh_->get_logger(),
                  "web socket has cache smart message num > 20, size %zu",
                  x3_smart_msg_.size());
    }
    if (x3_frames_.size() > 20) {
      RCLCPP_WARN(nh_->get_logger(),
                  "web socket has cache image num > 20, size %zu",
                  x3_frames_.size());
    }
  }
}
