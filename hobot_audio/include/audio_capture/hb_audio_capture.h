// Copyright (c) 2022，Horizon Robotics.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#ifndef INCLUDE_HBAUDIOCAPTURE_H_
#define INCLUDE_HBAUDIOCAPTURE_H_

#include <fstream>
#include <iostream>
#include <memory>
#include <string>
#include <thread>
#include <unordered_map>
#include <vector>

#include "utils/alsa_device.h"
#include "audio_msg/msg/smart_audio_data.hpp"
#include "rclcpp/rclcpp.hpp"
#include "std_msgs/msg/string.hpp"
#include <chrono>
#include <wiringPi.h>
#include <wiringSerial.h>
#include <softPwm.h>
#include "sensor_msgs/msg/imu.hpp"
#include "audio_engine/audioengine.h"

#define PWM_PIN 16

namespace hobot {
namespace audio {

using rclcpp::NodeOptions;
class HBAudioCapture : public rclcpp::Node {
 public:
  // node_name为创建的节点名，options为选项，用法和ROS Node相同
  HBAudioCapture(const std::string& node_name,
                 const NodeOptions& options = NodeOptions());

  
  virtual ~HBAudioCapture();

  int Run();


 public:
  int Init();
  int DeInit();
  int Start();
  int Stop();
  bool is_wake_up = false;  //是否唤醒
  int orientation;  //当前方位

  int initServo(int val) {
    wiringPiSetup();
    // 初始化舵机PWM引脚
    pinMode(PWM_PIN, OUTPUT);
    softPwmCreate(PWM_PIN, 0, 100);
    softPwmWrite(PWM_PIN, val);
    std::this_thread::sleep_for(std::chrono::milliseconds(400));
    RCLCPP_INFO(this->get_logger(), "Servo initialized to position: %d", val);
    return 0; // 返回初始化成功
  }
 private:
  int ParseConfig(std::string config_file);
  int MicphoneGetThread();
  void AudioDataFunc(char* buffer, int size);
  void AudioSmartDataFunc(float theta);
  void AudioCmdDataFunc(const char* cmd_word);
  void AudioEventFunc(int event);
  void AudioASRDataFunc(const char* asr);
  int Operate();   //串口发送函数
  // sendCommand函数声明
  void sendCommand(const std::string& cmd_word);
  void PublishTTSText(const std::string& text);
  std::string rotateAngleBy90Degrees(const std::string &angle_str);
  //void imu_callback(const sensor_msgs::msg::Imu::SharedPtr msg);

 private:
  int micphone_enable_ = 1;
  std::shared_ptr<std::thread> micphone_thread_;
  alsa_device_t* micphone_device_ = nullptr;
  bool exit_ = true;
  bool is_init_ = false;
  int audio_num_ = 0;
  std::string micphone_name_ = "hw:0,0";
  int micphone_rate_ = 16000;
  int micphone_chn_ = 8;
  int micphone_buffer_time_ = 0;
  int micphone_nperiods_ = 4;
  int micphone_period_size_ = 512;
  int voip_mode_ = 0;
  int mic_type_ = 0;
  int asr_output_mode_ = 0;
  int asr_output_channel_ = 3;

  int au_doa;
  // 抓取控制
  //auto message1 = std::make_unique<std_msgs::msg::String>();
  //std::unique_ptr<std_msgs::msg::String> message1 = std::make_unique<std_msgs::msg::String>();
  
  // 语音播报
  //auto message2 = std::make_unique<std_msgs::msg::String>();
  //std::unique_ptr<std_msgs::msg::String> message2 = std::make_unique<std_msgs::msg::String>();

  std::string serial_data_1 = "";

  std::string config_path_ = "./config";
  // audio_sdk_path_ will be updated at runtime with env "TROS_DISTRO"
  std::string audio_sdk_path_ = "";
  std::string audio_pub_topic_name_ = "/audio_smart";
  std::string asr_pub_topic_name_ = "/audio_asr";
  std::ofstream audio_infile_;
  std::ofstream audio_sdk_;
  bool save_audio_ = false;

  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr tts_text_publisher_;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr cmd_publisher_;
  rclcpp::Publisher<audio_msg::msg::SmartAudioData>::SharedPtr msg_publisher_;
  rclcpp::Publisher<std_msgs::msg::String>::SharedPtr asr_msg_publisher_;

  //rclcpp::Subscription<sensor_msgs::msg::Imu>::SharedPtr imu_subscription_;

};

}  // namespace audio
}  // namespace hobot

#endif
